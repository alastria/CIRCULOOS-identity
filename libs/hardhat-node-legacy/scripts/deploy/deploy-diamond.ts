...
export async function deployDiamond(options: DeployOptions = {}) {
  ...
  // Detect environment: 'docker' or 'default'
  const networkName = (network as any).name || 'hardhat'
  const detectedEnv = (process.env.DEPLOY_ENV as 'default' | 'docker') || environment || 'default'
  ...
  // 7. Save to Shared Config JSON (for Docker/K8s)
  if (process.env.SHARED_CONFIG_PATH) {
    await saveSharedConfig(diamond.address, network, process.env.SHARED_CONFIG_PATH)
  }
...
async function updateCheckpoint(environment: 'default' | 'docker', checkpoint: Partial<Checkpoint>) {
  try {
    const rootDir = path.resolve(__dirname, '../../../..')
    const checkpointPath = path.join(rootDir, `packages/hardhat-node/.checkpoint.${environment}.json`)
...
async function saveDiamondAddressToAllServices(diamondAddress: string, environment: 'default' | 'docker') {
  const rootDir = path.resolve(__dirname, '../../../..')

  // Services that need DIAMOND_ADDRESS (corrected paths)
  const services = [
    { name: 'hardhat-node', path: path.join(rootDir, 'packages/hardhat-node') },
    { name: 'issuer', path: path.join(rootDir, 'backend/issuer') },
    { name: 'verifier', path: path.join(rootDir, 'backend/verifier') },
    { name: 'web', path: path.join(rootDir, 'frontend') }
  ]

  // Determine .env file name based on environment
  const envFileName = environment === 'docker' ? '.env.docker' : '.env'
...
async function saveSharedConfig(diamondAddress: string, network: any, configPath: string) {
  try {
    const config = {
      diamondAddress,
      deployedAt: new Date().toISOString(),
    ...
    console.log(`\n💾 Saved Shared Config to ${configPath}`)
  } catch (error) {
    console.warn(`⚠️  Could not save shared config to ${configPath}:`, error)
  }
}
