...
library LibDiamondCut {
    ...
    function removeFunctions(address _facetAddress, bytes4[] memory _functionSelectors) internal {
        require(_functionSelectors.length > 0, "LibDiamondCut: No selectors in facet to cut");
        LibDiamondStorage.DiamondStorage storage ds = LibDiamondStorage.diamondStorage();
        // if function does not exist then it won't be in the map, so removeFunction will fail
    ...
    function removeFunction(LibDiamondStorage.DiamondStorage storage ds, address _facetAddress, bytes4 _selector) internal {        
        ...
        // if not, we need to replace the selector being deleted with the last selector
